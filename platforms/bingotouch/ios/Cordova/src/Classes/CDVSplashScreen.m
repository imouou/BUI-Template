...
- (void)createViews
{
    ...
    [parentView addObserver:self forKeyPath:@"frame" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
...
- (void)destroyViews
{
    ...
    NSLog(@"%@%@%@",self,NSStringFromClass([self class]),NSStringFromSelector(_cmd));
...
- (void)updateBounds
{
    ...
    CGRect screenFrame = [self.viewController.view convertRect:[UIScreen mainScreen].bounds fromView:nil];

    // There's a special case when the image is the size of the screen.
    if (CGSizeEqualToSize(screenFrame.size, imgBounds.size)) {
//        CGRect statusFrame = [self.viewController.view convertRect:[UIApplication sharedApplication].statusBarFrame fromView:nil];
//        imgBounds.origin.y -= statusFrame.size.height;
        imgBounds = screenFrame;
    } else {
